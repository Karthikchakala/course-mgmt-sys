...
const LoginPage = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // Redirection is handled by the initial check after context updates
        } catch (err) {
                ...
                <form onSubmit={handleSubmit}>
                    <div className="mb-5">
                        ...
                        <input
                            type="email"
                            id="email"
                            placeholder="admin@cms.com or student@user.com"
                            value={email}
                            onChange={(e) => setEmail(e.target.value)}
                            className="w-full px-4 py-2 border border-gray-300 rounded-lg text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-500 transition duration-150"
                            required
                        />
                    </div>
                    <div className="mb-6">
                    ...
                    <button
                        type="submit"
                        className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-2.5 px-4 rounded-lg focus:outline-none focus:shadow-outline transition duration-150 shadow-md"
                    >
                        Login
                    </button>
                </form>
                <p className="text-center text-gray-500 text-sm mt-6">
                    New student? <Link to="/register" className="text-blue-600 hover:text-blue-800 font-medium">Create an account</Link>
                </p>
            </div>
        </div>
    );
