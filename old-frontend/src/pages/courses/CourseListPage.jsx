...
const CourseListPage = () => {
    ...
    useEffect(() => {
        const fetchCourses = async () => {
            try {
                ...
                console.error('Failed to fetch courses:', err);
                setCourses([]);
                setError('Failed to load courses. Check backend and network.');
            } finally {
                setLoading(false);
            }
        };
    ...
    if (loading) {
        return <p className="text-center mt-20 text-xl">Loading courses...</p>;
    }
    ...
    return (
        <div className="container mx-auto p-8">
            <h2 className="text-4xl font-bold mb-8 text-center">Available Courses</h2>
            {courses.length === 0 ? (
                <p className="text-center text-gray-600">No courses are currently available.</p>
            ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {courses.map(course => (
                        <div key={course.id} className="p-6 border rounded-lg shadow hover:shadow-lg transition">
                            <h3 className="text-xl font-bold mb-2">{course.title}</h3>
                            <p className="text-gray-700 mb-2">{course.description}</p>
                            <p className="font-semibold text-green-600 mb-2">₹{course.price}</p>
